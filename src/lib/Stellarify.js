...
const Stellarify = {
    ...
    parseAssetSlug(slug) {
        ...
        if (code.length > 12) {
            throw new Error(
                `Stellarify.parseAssetSlug expected asset code to be 12 or fewer characters. Input: ${code}`,
            );
        }
    ...
    rectifyOffer(baseBuying, counterSelling, offer) {
        ...
        if (baseBuying.equals(offerBuying) && counterSelling.equals(offerSelling)) {
            ...
            newOffer.baseAmount = new BigNumber(offer.amount)
                .dividedBy(new BigNumber(newOffer.price_r.n).dividedBy(newOffer.price_r.d))
                .toFixed(7);
            ...
            newOffer.counterAmount = new BigNumber(newOffer.price_r.n)
                .dividedBy(newOffer.price_r.d)
                .times(offer.amount)
                .toFixed(7);
        } else {
