...
const Validate = {
    ...
    address(input, type) {
        ...
        // - Includes all valid addresses and a few invalid ones too such as fake TLD or misuse of hyphens
        // or excessive length
    ...
    muxedKey(input) {
        ...
        try {
            const base = StellarSdk.MuxedAccount.fromAddress(input, '0').baseAccount().accountId();

            if (!StellarSdk.StrKey.isValidEd25519PublicKey(base)) {
                return resultError('Invalid muxed key');
            }
            return RESULT_VALID;
        } catch (e) {
            return resultError('Invalid muxed key');
