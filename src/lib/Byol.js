export default function Byol() {
  let events = {}; // Build your own listener
  ...
  this.listen = (eventName, cb) => {
    ...
    let listenerIndex = events[eventName].nextIndex;
  ...
  this.trigger = (eventName) => {
    ...
    for (let i = 0; i < events[eventName].nextIndex; i++) {
      let listener = events[eventName].listeners[i];
      if (listener) {
        listener();
      }
    }
