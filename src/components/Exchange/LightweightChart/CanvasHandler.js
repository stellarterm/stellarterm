...
function drawChartData(ctx, w, h, pairName, timeFrameInMin, trade, volume) {
    ...
    const ohlcString = `O:  ${fillWithZeros(trade.open)}  H:  ${fillWithZeros(trade.high)}  L:  ${fillWithZeros(trade.low)}  C:  ${fillWithZeros(trade.close)}  V:  ${getReadableVolume(volume.value)}`;
    ctx.fillStyle = '#999999';
