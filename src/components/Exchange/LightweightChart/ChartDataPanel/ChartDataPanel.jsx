        ...
        return (
            <div className="data_Panel">
                <div className="chart_Data">
                    ...
                    <span className={spanClass}>{getReadableNumber(trade.open)}</span>
                    <span className="data_Title">H:</span>{' '}
                    <span className={spanClass}>{getReadableNumber(trade.high)}</span>
                    <span className="data_Title">L:</span>{' '}
                    <span className={spanClass}>{getReadableNumber(trade.low)}</span>
                    <span className="data_Title">C:</span>{' '}
                    <span className={spanClass}>{getReadableNumber(trade.close)}</span>
                    <span className="title_Volume">Volume ({volume.volumeName}):</span>{' '}
                    <span className={`data_Volume ${spanClass}`}>{getReadableNumber(volume.value)}</span>
                </div>
            </div>
        );
    }
