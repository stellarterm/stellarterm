...
import SendStepAmountInput from './SendStepAmountInput';
import SendStepEditLink from './SendStepEditLink';

export default class SendStepAmount extends React.Component {
    getStepContent() {
        ...
        return (
            <div className={`Send__panel${!isCurrentStep && !stepIsPassed ? ' is-future' : ''}`}>
                <h3 className={`Send__title${isCurrentStep ? ' is-active' : ''}`}>
                    3. Amount <SendStepEditLink stepIsPassed={stepIsPassed} editStep={step3Edit} />
                </h3>
                {stepContent}
            </div>
        );
