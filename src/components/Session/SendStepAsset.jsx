...
export default class SendStepAsset extends React.Component {
    getStepContent() {
        ...
        return (
            <div className={`Send__panel${!isCurrentStep && !stepIsPassed ? ' is-future' : ''}`}>
                <h3 className={`Send__title${isCurrentStep ? ' is-active' : ''}`}>
                    2. Asset <SendStepEditLink stepIsPassed={stepIsPassed} editStep={step2Edit} />
                </h3>
                {stepContent}
            </div>
        );
