...
export default class AssetCard2 extends React.Component {
  render() {
    if (!this.props.code) {
      throw new Error(
        "AssetCard2 expects to get a code in the props. Instead, got: " +
          this.props.code
      );
    }
    ...
    if (this.props.domain !== undefined) {
      ...
      asset = directory.resolveAssetByAccountId(
        this.props.code,
        this.props.issuer
      );
    } else {
    ...
    let issuerAccountId =
      asset.issuer === null ? "native lumens" : asset.issuer;
    ...
    let assetCardMain = (
      <div className="AssetCard2__main" style={backgroundStyle}>
        <img className="AssetCard2__logo" src={anchor.logo} />
    ...
    if (this.props.children) {
      ...
      return (
        <div className={containerClassName} style={borderStyle}>
          {assetCardMain}
          <div
            className="AssetCard2__addon"
            style={Object.assign({}, borderStyle, backgroundStyle)}
          >
            {this.props.children}
          </div>
        </div>
